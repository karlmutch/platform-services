...
func GetUserFromClaims(ctx context.Context) {
	...
	user, err := func() (user string, err kv.Error) {
		if auth := md.Get("authorization"); len(auth) > 0 {
			splitToken := strings.Split(auth[0], "Bearer")
			if len(splitToken) != 2 {
				return "", kv.NewError("badly formatted token").With("stack", stack.Trace().TrimRuntime())
			}
...
func runServer(ctx context.Context, tracer *zipkin.Tracer, serviceName string, ipPort string) (errC chan kv.Error) {
